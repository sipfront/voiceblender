...
| `SIP_CONTACT_USER_MODE` | `none` | User part of the `Contact` VoiceBlender advertises. `none` — `<sip:host:port>`, no user part. `fixed` — the literal in `SIP_CONTACT_USER`, on every dialog. `local` — this side's own identity: the `To` user of a request arriving here, the `From` user of one we originate; on a back-to-back user agent that is the identity each leg stands in for. Set this when a peer mishandles a userless `Contact` — some synthesise a user part of their own and then route or match on it. |
| `SIP_CONTACT_USER` | *(empty)* | The literal user part used when `SIP_CONTACT_USER_MODE=fixed`. |
