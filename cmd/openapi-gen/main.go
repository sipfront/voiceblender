...
func configVars() *seq {
	vars := []configVar{
		...
		{Name: "SIP_CONTACT_USER_MODE", Default: "none", Description: "User part of the Contact advertised in SIP requests and responses. none: <sip:host:port>, no user part. fixed: the literal in SIP_CONTACT_USER, on every dialog. local: this side's own identity, meaning the To user of a request arriving here and the From user of one we originate. Set this when a peer mishandles a userless Contact, as some synthesise a user part of their own and then route or match on it."},
		{Name: "SIP_CONTACT_USER", Default: "", Description: "The literal user part used when SIP_CONTACT_USER_MODE is fixed."},
