...
// callers that must act per audio source rather than per leg. A recording
// session is the case that needs it: its m= sections are other parties' audio,
// so the room holds two stream participants and no leg participant at all.
type StreamParticipant struct {
	// ParticipantID is the mixer's key for this stream, "legID/streamID".
	ParticipantID string
	LegID         string
	StreamID      string
	// Role is what the stream was attached as — for a recording session, the
	// AOR of the participant whose audio it carries. It can be updated later by
	// a metadata refresh, so treat it as a hint and resolve identity from the
	// leg's recording-session view when it matters.
	Role string
}
