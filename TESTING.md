...
| `internal/stt` (timings) | 2 | A transcript carries where in the audio it was said: Flux takes the first and last word's own timings where a turn has words and its audio window otherwise (the window opens before the speaker does), a partial carries the same span as the final, and Deepgram v1's `start`/`duration` wire names are pinned — a typo in a JSON tag is a silent zero that would seek every line to the start of the call |
| `internal/stt` (flux model) | 2 | `language_hints` with no model named selects `flux-general-multi`, the only model Deepgram accepts them on; an explicitly named `flux-general-en` wins and the hints are dropped rather than the dial being refused |
