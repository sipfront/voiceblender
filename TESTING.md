...
| `TestMultiChannel_RoomPlaybackGetsItsOwnChannel` | Audio played into a recorded room (`POST /v1/rooms/{id}/play`) gets its own channel in the multi-channel file, keyed by the playback ID, while both legs keep theirs. The mix already contains a playback source — it is a mixer participant — so this is the per-party file agreeing with it. |
| `TestMultiChannel_LegPlaybackGetsItsOwnChannel` | Audio played to one leg (`POST /v1/legs/{id}/play`) also gets its own channel, teed from what the room mixer is handed. It stays out of the mix, which is a record of what the room heard and not of what one party was played. |
| `TestMultiChannel_RoomTTSGetsItsOwnChannel` | A line synthesized into a recorded room (`POST /v1/rooms/{id}/tts`) gets its own channel, keyed by the TTS ID, while both legs keep theirs — the same arrangement a room playback gets, for the same reason. |
| `TestMultiChannel_LegTTSGetsItsOwnChannel` | A line spoken to one leg (`POST /v1/legs/{id}/tts`) gets its own channel, teed the way a leg playback is, and stays out of the mix. Without it a one-sided spoken announcement was in no file at all. |
| `TestMultiChannel_TTSIsNotRecordedUnlessAsked` | Neither TTS path is in the multi-channel file without `record`, which is what every caller written before the flag gets. |
