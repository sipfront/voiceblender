...
| `internal/siprec` (verify) | 15 | `Verify` cross-checks a metadata document against the SDP it arrived with: labels agreeing with the offer, caller/callee inverted (`participant_mismatch` from `a=ssrc cname`), cname host differing from the AOR domain, absent and non-URI cnames, duplicate labels, a label the offer does not carry, a section nobody sends on, an offer with no labels and no offer at all (nothing to compare, so nothing reported), hostless `tel:` URIs on both sides, a disassociated sender, nil recording, and `aorUser` parsing |
| `internal/sip` (a=ssrc) | 2 | `ParseSDP` keeps the `a=ssrc … cname:` of each m= section alongside its `a=label` — the offer's own statement of who sends on it — and `ssrcCNAME` handles attributes with no cname, no value and no attribute part |
